...
nd = 6;  %Number of information OFDM symbol for one loop  %每个子载波调制的信息
...
for iii = 1:nloop
    ...
    %信号的功率大小 默认均值为零，同样可以用方差来计算
    spow1 = sum(sum(ich1.^2+ qch1.^2))/nd./para;
    spow1_var = var(reshape(x,[],1));
    
    y = ifft(x);
    ich2 = real(y);
    qch2 = imag(y);
    
    spow2 = sum(sum(ich2.^2+ qch2.^2))/nd./para;  %经过傅里叶反变换后，功率变为原来的1/N
    spow2_var = var(reshape(y,[],1));
...
%     hold on;
    ...
    %加入了保护间隔，但取平均时还是之前的没有加入保护间隔的长度
    spow = sum(ich3.^2+ qch3.^2)/nd./para;
    %运用公式 snr = ebn0-10log10（1/2*Tsym/Tsample）,计算出噪声功率
    attn = 0.5*spow*sr/br*10.^(-ebn0/10);
    %根据顺势功率公式计算出噪声大小 U = I^2*r
    attn = sqrt(attn);
