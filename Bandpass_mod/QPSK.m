...
%% 参数初始化
% 重要转换
%采样周期 = 符号周期 * 采样点数  (Tsample = Tsym * sps)
...
%     freqz(idata_mes)
    ...
    delay_point = 96*2/2;
    rcos_idata_sample = idata_mes(delay_point:sps:end);
    rcos_qdata_sample = qdata_mes(delay_point:sps:end);
    ...
    %误码率检验
    reciver_data = zeros(1,990);
    reciver_data(1:2:end) = rcos_idata_final;
    reciver_data(2:2:end) = rcos_qdata_final;
    [number(i) ,ratio_err(i)] = biterr(reciver_data,data(1:length(reciver_data)));
end
